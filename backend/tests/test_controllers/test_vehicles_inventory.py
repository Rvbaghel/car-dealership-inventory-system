...
def test_purchase_vehicle_requires_authentication(client):
    """
    TDD Purchase Test 1 (RED): Enforces that the purchase endpoint cannot be accessed 
    without a valid structural Authorization header.
    """
    vehicle_id = 1
    purchase_payload = {"quantity": 1}
    
    # Path A: Missing Authorization Header entirely
    response_missing = client.post(f"/api/vehicles/{vehicle_id}/purchase", json=purchase_payload)
    assert response_missing.status_code == status.HTTP_401_UNAUTHORIZED
    assert response_missing.json()["detail"] == "Missing or invalid Authorization header structural format"
